...
for small_em=1:repeat_small_em
    ...
    for i=1:n_small_em
        ...
        [Phi_pre(:,:,i),Responsibilities,pi_new, Sigma_pre(:,:,:,i),loglikelihood,info] = cemgm_estimate (X,Y,Phi_init,[],modes,emMaxIter_pre,maxErr_pre);
...
Phi_init_chosen_cem=Phi_init_chosen(:,:,indices(end));
Sigma_init_chosen_cem=Sigma_init_chosen(:,:,:,indices(end));

[Phi,Responsibilities,pi_new, Sigma,loglikelihood,info] = emgm_estimate (X,Y,Phi_init_chosen_cem,Sigma_init_chosen_cem,modes,emMaxIter,maxErr);
...
for small_em=1:repeat_small_em
    ...
    for i=1:n_small_em
        ...
        [Phi_tilde_pre(:,:,i),Responsibilities_tilde,~,Sigma_tilde(:,:,:,i),loglikelihood_tilde,info_tilde] = cemgm_estimate (X,Y,Phi_init_tilde,[],modes,emMaxIter_pre,maxErr_pre);
    ...
    loglikelihoods_cem_tilde(:,small_em)=loglikelihoods_tilde_pre(indices(end));
end

[~, indices] = sort(loglikelihoods_cem_tilde);
Phi_init_chosen_cem_tilde=Phi_init_chosen_tilde(:,:,indices(end));
Sigma_init_chosen_cem_tilde=Sigma_init_chosen_tilde(:,:,:,indices(end));

[Phi_tilde,Responsibilities_tilde,~,Sigma_tilde,loglikelihood_tilde,info_tilde] = emgm_estimate (X,Y,Phi_init_chosen_cem_tilde,Sigma_init_chosen_cem_tilde,modes,emMaxIter,maxErr);

[~,indices]=sort(sum(Phi_tilde.^2,2));
Phi_tilde=Phi_tilde(flip(indices),:);
Phi_init_chosen_cem_tilde
Phi_init_orig_tilde
Phi_tilde
