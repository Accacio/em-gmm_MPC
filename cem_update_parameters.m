function [Phi, pi_new, Sigma] = cem_update_parameters(X, Y, OldPhi, Responsibilities)
% UPDATE_PARAMETERS -
    ...
    nu=d+2;
    zeta=2*sigma0^(1/d);
    [~,index] = max(Responsibilities);

    Responsibilities=zeros(modes,N);
    for i=1:N
        Responsibilities(index(i),i)=1;
    end
    % Reponsibilities=Responsibilities./max(Responsibilities)>=1;
    for i=1:modes
